...
class PerpBitget():
    ...
    def fetch_ohlcv_wrapped(self, symbol, timeframe, limit):
        print(f"Fetch OHLCV pour {symbol}-{timeframe}")
        return self._session.fetch_ohlcv(symbol, timeframe, limit=limit)
    
    def get_last_historical(self,symbol, timeframe, limit, since=None):
        result = pd.DataFrame(data=self._session.fetch_ohlcv(
            symbol, timeframe, since, limit=int(limit)))
    ...
    def fetch_position_thicker(self, symbol):
        try:
            pos = []
    ...
    @authentication_required
    def place_market_order_direct(self, symbol, side, amount, reduce=False):
        try:
            self._session.set_position_mode( False, symbol )
            return self._session.create_order(symbol,'market',side,amount)
        except BaseException as err:
            raise Exception(err)

    @authentication_required
    def close_open_positions(self, symbol, with_retry = False):
        try:
            self._session.options['createMarketBuyOrderRequiresPrice'] = False
            self._session.options['createMarkeSellOrderRequiresPrice'] = False
            positions = self._session.fetch_positions()
            for position in positions:
                sym = position['symbol']
                pos_t = symbol + ":USDT"
                if pos_t == sym:
                    print(f"Position {position['side']} fermée pour {pos_t} - {position['side']} - {position['contracts']} - {position['entryPrice']}")
                    self._session.close_position(pos_t,None, params={'reduce_only' : True})
        except Exception as e:
            print(f"Erreur lors de la fermeture des positions ouvertes : {e}")
            print(f"Retry : {with_retry}")
            if(with_retry):
                time.sleep(5)
                positions = self._session.fetch_positions()
                for position in positions:
